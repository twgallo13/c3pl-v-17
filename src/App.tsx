...
function Guarded(props: { path: string; children: React.ReactNode }) {
    const role = getRole();
    const access = checkAccess([role], []);
    return access.allowed ? <>{props.children}</> : <Navigate to={landingFor(role)} replace />;
}
            ...
            <AppShell version={VERSION}>
                <React.Suspense fallback={<div className="p-4">Loading…</div>}>
                    <Routes>
                        ...
                        <Route
                            path="/products"
                            element={<Guarded path="/products"><ProductsView /></Guarded>}
                        />
                        <Route
                            path="/products/:id"
                            element={
                                <Guarded path="/products/:id">
                                    <ProductDetail />
                                </Guarded>
                            }
                        />
                        {/* Basic routes without ROUTES registry */}
                        <Route
                            path="*"
                            element={
