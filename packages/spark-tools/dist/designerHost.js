...
function handleClick(event) {
    ...
    // let node = fiberNode;
    // while (node.return) {
    //   const { type, memoizedProps } = node.return;
    //   let ancestorData = {
    //     type: type?.name || type,
    //     props: extractProps(memoizedProps),
    //     location: {},
    //   };
    //   if (memoizedProps?.["data-loc"]) {
    //     const [filePath, line, column] = memoizedProps["data-loc"].split(":");
    //     ancestorData = {
    //       ...ancestorData,
    //       location: {
    //         type: type?.name || type,
    ...
    //     };
    //   }
    //   ancestorList.unshift(ancestorData);
    //   node = node.return;
    // }
    const elementDynamic = element.getAttribute('data-dynamic');
    const isTextElement = typeof fiberProps.children === 'string';
...
function handleMessage(message) {
    switch (message.type) {
        case 'spark:designer:bridge:enable': {
        ...
        case 'spark:designer:bridge:update-class-name': {
            ...
            document.querySelectorAll(`[data-loc="${filePath}:${line}:${column}"]`).forEach((el) => {
                ...
                const newClassName = replace ? className : twMerge(elementClassName, className);
